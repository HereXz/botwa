...
haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption:
 `
 *@${num.split("@")[0]}* Selamat Datang  Di Group *${metadata.subject}* 
 
 Saya adalah *Ryo, Bot WhatsApp.*
`})
...
haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Ciee Jadi Admin Di Group ${metadata.subject}`  })
} else if (anu.action == 'demote') {
haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `*@${num.split('@')[0]}* Ciee Di Unadmin, Di Group ${metadata.subject}`})
  }
}
} catch (err) {
console.log(err)
...
start(`1`,`Connecting...`)
  ...
  haikal.sendMessage('6285731947500' + "@s.whatsapp.net", { text: `*Ryo Versi 2*
